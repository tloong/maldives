class PrintServices2 < Struct.new(:pyear, :pmonth, :pis_mortgaged)
      ...
      ff.each do |ffe|
        ...
        if (month_difference(e_date,query_date)==0)
          ...
          if (this_year > ffe.start_use_date)
            ...
            data_hash[index][ffe.id][15] = data_hash[index][ffe.id][16]
          end
      ...
      f_redep.each do |ffe|
        ...
        if (month_difference(ffe.re_end_use_date,query_date)==0)
          ...
          data_hash[index][fid][16] = data_hash[index][fid][16] + ffe.re_depreciated_value_per_month * (month_difference(query_date,ffe.re_start_use_date)) + ffe.re_depreciated_value_last_month 
            ...
            if (fid ==185124)
              puts "md = #{month_difference(ffe.re_end_use_date,ffe.re_start_use_date)}"
              Delayed::Worker.logger.debug("md = #{month_difference(ffe.re_end_use_date,ffe.re_start_use_date)}")
            end
          ...
          if (this_year > ffe.re_start_use_date)
            ...
            data_hash[index][fid][15] = data_hash[index][fid][16]
          end
